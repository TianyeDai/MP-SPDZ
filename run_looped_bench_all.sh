...
########################################
PROGRAM="${PROGRAM:-loop_unrolled_linear}"            # name passed to compile.py and runtime
...
LGP="${LGP:-32}"                             # -lgp for Fake-Offline (prime bit-length)
FLAGS="-v -F -N 2 -S ${SECURITY} -lgp 32 -h ${HOST}" # -v prints setup timer

# compile.py flags
COMPILE_FIELD_BITS="${COMPILE_FIELD_BITS:-32}"  # ./compile.py -F <bits> <prog>
...
for n in "${SIZES[@]}"; do
  ...
  echo "[2/5] Preprocess (Fake-Offline) for ${PROGRAM} | parties=2, lgp=${LGP}, S=${SECURITY}"
  ./Fake-Offline.x 2 -lgp "${LGP}" -S "${SECURITY}" -p "${PROGRAM}"

  echo "[3/5] Per-thread compile + runs"
  for t in "${THREAD_SET[@]}"; do
    echo "  -> THREADS=${t}"
    fe_sec="$(compile_and_time "${n}" "${t}" "${COMPILE_FIELD_BITS}" "${PROGRAM}")"
    fe_sec="${fe_sec//$'\r'/}"
    fe_sec="${fe_sec//$'\n'/}"
